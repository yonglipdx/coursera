...
int ladder_length(vector<string>& input, string& start, string& end){
   ...
   while(!found && !stack1.empty()){
      level++;
      while(!stack1.empty()){
         string this_word{stack1.top()};
         if (this_word == end) return level;
         for (int i=0; i< this_word.length(); i++){
            for (char j = 'a'; j <= 'z'; j++){
               string temp_word{this_word};
               temp_word[i] = j;
               if (temp_word == end) return ++level;
               if (find(dict.begin(), dict.end(),temp_word)!= dict.end() && !(visited.count(temp_word)>0)){
               //if (!(visited.count(temp_word)>0)){ ----> in case fake words are allowed. but in this case we should only compared different character in the two words.
                  visited.insert(temp_word);
                  stack2.push(temp_word);
               }
            }
         }
         stack1.pop();
